...
// Get all work for a specific minion by minionId
const getAllWorkForMinion = (minionId) => {
  return db.allWork.data.filter(workItem => workItem.minionId === minionId);
}

// Add new work to a minion's backlog
const addWorkToMinion = (minionId, workData) => {
  const newWork = { ...workData, id: `${workIdCounter++}`, minionId };
  db.allWork.data.push(newWork);
  return newWork;
}

// Get work by ID for a specific minion
const getWorkById = (minionId, workId) => {
  return db.allWork.data.find(workItem => workItem.minionId === minionId && workItem.id === workId);
}

// Update work by ID for a specific minion
const updateWorkForMinion = (minionId, workId, updatedWorkData) => {
  const workIndex = db.allWork.data.findIndex(workItem => workItem.minionId === minionId && workItem.id === workId);
  if (workIndex === -1) return null;
  const updatedWork = { ...db.allWork.data[workIndex], ...updatedWorkData };
  db.allWork.data[workIndex] = updatedWork;
  return updatedWork;
}

// Delete work by ID for a specific minion
const deleteWorkForMinion = (minionId, workId) => {
  const workIndex = db.allWork.data.findIndex(workItem => workItem.minionId === minionId && workItem.id === workId);
  if (workIndex === -1) return false;
  db.allWork.data.splice(workIndex, 1);
  return true;
}

// Update the `db` export to include the new functions
module.exports = {
  createMeeting,
  getAllFromDatabase,
  getFromDatabaseById,
